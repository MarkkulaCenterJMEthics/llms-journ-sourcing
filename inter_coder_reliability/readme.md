...
**Beyond zero-variability: a skewed-but-nonzero category distribution can also pull Krippendorff's Alpha down sharply, in a way AC1 resists.** Verified on the combined `94-109-SZ.csv`/`94-109-AV.csv` set (23 gated items, `Type of Source` distribution `Named Person`=43, `Named Organization`=1, `Unnamed Group of People`=2, only 1 real disagreement): Krippendorff's Alpha comes out to `0.6565` while AC1 comes out to `0.9556` on the *same* data. Cause: `De` sums `freq[c] × freq[k] × distance(c,k)` over every class pair — a single rare-category rating gets multiplied against all 43 `Named Person` ratings, so one lonely occurrence generates a large pairwise-disagreement count even though nothing about actual annotator agreement changed. AC1's `pe` is built from category *proportions* (`Σ pᵢ(1-pᵢ)`) instead, so a dominant category just makes `pe` small — it doesn't inflate with scale the way Krippendorff's `De` does. This is the documented "kappa paradox": count-based chance-correction can be misleadingly punishing under skewed category prevalence; Gwet built AC1 specifically to resist it.

Also verified numerically: **adding more data with the same skewed proportions and disagreement rate does not fix this** — Krippendorff's Alpha converges to a fixed value (`≈0.649` here) as `N` grows, not toward `1.0`, since `De` scales quadratically with `N` while `Do` only scales linearly. What actually helps is the category *mix* becoming genuinely less skewed, not sample size alone.
