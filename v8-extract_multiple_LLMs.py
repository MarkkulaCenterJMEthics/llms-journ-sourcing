...
def process_files(input_dir, output_dir):
    ...
    for i in range(2):  # Loop to run the experiment 5 times
        ...
        for input_file in glob.glob(os.path.join(input_dir, "*.txt")):
            ...
            try:
                deepseek_response = analyze_with_openrouter(
                    article_text, "deepseek/deepseek-r1"
                )
                save_json_and_csv(
                    deepseek_response, "deepseek-r1", input_file, experiment_dir, i + 1
                )
                time.sleep(30)
            except Exception as e:
            ...
            try:
                gemini_response = analyze_with_openrouter(
                    article_text, "google/gemini-2.5-flash-preview"
                )
                save_json_and_csv(
                    gemini_response,
                    "gemini-2.5-flash",
